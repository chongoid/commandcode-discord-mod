...
export interface RequestRecord {id: string; conversationId: string; sourceMessageId: string; prompt: string; attachments?: AttachmentInfo[]; state: RequestState; queuePosition?: number; acceptedAt: number; startedAt?: number; finishedAt?: number; attemptId?: string; statusMessageId?: string; finalText?: string; error?: string; sessionIdAtStart?: string}
...
export type OutboxKind = 'status' | 'final' | 'notice' | 'file';
export interface OutboxItem {id: string; conversationId: string; requestId?: string; kind: OutboxKind; operation: 'send' | 'edit'; content: string; messageId?: string; chunkIndex?: number; files?: string[]; state: OutboxState; deliveredAt?: number; attempts: number; nextAttemptAt: number; lastError?: string; createdAt: number}
